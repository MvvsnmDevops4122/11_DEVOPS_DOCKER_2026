...
## Sample Shell Script (Get Start Time)

Create file : get_start_time.sh (chmod +x get_start_time.sh)
...
if [ $? -eq 0 ]; then
    echo "Container '$container' started at: $start_time"
else
    echo "Failed to get start time. Please check the container name."
fi
````

Run: ./get_start_time.sh

---

## Script: Pass Dynamic Field
...
echo "Enter docker inspect field (e.g., .State.StartedAt):"
read field

value=$(docker inspect --format "{{${field}}}" "$container")

echo "Field '$field' of container '$container': $value"
```
...
* A dangling image is a Docker image layer that is not tagged and not referenced by any container.
...
* docker logs <container_id_or_name>                      # View all logs

* docker logs --tail <number_of_lines> <container_name>   # View last N lines
...
* exit – to come out of the container shell.

it means intractive mode
...
Yes – You can limit container memory using the --memory flag.

* docker run --memory="512m" -d -p 8083:8080 --name ONE satyamolleti4599/maven-web-app:1.0.0

* docker run --memory="64m" -d  -p 8084:8080 --name FIVE satyamolleti4599/maven-web-app:1.0.0  --> Here container will be killed.

* docker inspect <cid> --> OOMKilled (outof memory)

---

##  Docker Commit

* Used to save the current state of a running container as a new image.

* docker commit <container_name_or_id> <repo_name>:<tag>

EX: docker commit ONE satyamolleti4599/java-web-app:24
